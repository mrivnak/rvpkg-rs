...
fn test_read_neofetch() {
    ...
    assert_eq!(package.definition.provides, nonempty!["neofetch".to_string()]);
    assert_eq!(package.definition.architectures, nonempty![PackageArchitecture::Any]);
    ...
    assert_eq!(package.build.build.lines().map(|l| l.trim()).collect::<Vec<&str>>()[0], "tar -xf neofetch-7.1.0.tar.gz");
    let install_lines = package.build.install.lines().map(|l| l.trim()).collect::<Vec<&str>>();
    assert_eq!(install_lines[0], "cd neofetch-7.1.0");
    assert_eq!(install_lines[1], "make DESTDIR=$OUTDIR install");
    assert_eq!(install_lines[2], "install -Dm644 LICENSE $OUTDIR/usr/share/licenses/neofetch/LICENSE");
...
fn test_read_arch_any_and_other() {
    assert!(read_package(Path::new("tests/examples/invalid/arch_any_and_other.rvbuild")).is_err());
}
...
fn test_insert_variables() {
    ...
    let install_lines = package.build.install.lines().map(|l| l.trim()).collect::<Vec<&str>>();
    assert_eq!(install_lines[0], "cd neofetch-7.1.0");
    assert_eq!(install_lines[1], "make DESTDIR=/var/tmp/rvbuild/neofetch install");
    assert_eq!(install_lines[2], "install -Dm644 LICENSE /var/tmp/rvbuild/neofetch/usr/share/licenses/neofetch/LICENSE");
    assert_eq!(package.build.post_install, None);
}